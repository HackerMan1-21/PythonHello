...
def get_thumb_cache_file(folder):
    # サムネイルキャッシュファイル名生成（常にfolder必須、Noneなら'global'扱い）
    if folder is None:
        folder = 'global'
...
def get_image_thumbnail(filepath, size=(240,240)):
    # サムネイル生成（キャッシュ用/表示用）
    # size=(240,240): キャッシュ用デフォルト, (140,140)や(80,80)は表示用
    key = (filepath, size)
    with thumb_cache_lock:
...
class DuplicateFinderGUI(QWidget):
    ...
    def find_duplicates(self):
        ...
        for _ in thumb_workers:
            thumb_q.put(None)
        # --- キャッシュ一度だけ保存 ---
        save_cache(cache_file, pickle.dumps(cache_dict))
    ...
    def delete_selected(self):
        if not self.selected_paths:
            self.progress_time_label.setText("ファイルを選択してください")
            return
        # 削除方法選択ダイアログ
        ...
        for path in list(self.selected_paths):
            try:
                if os.path.isdir(path):
                    ...
                    failed.append(f"{path} : ファイル/ディレクトリが存在しません")
            except Exception as e:
    ...
    def move_selected_files_to_folder(self, checkboxes):
        ...
        for cb, path in checkboxes:
            if not cb.isChecked():
                continue
            if not path or not os.path.isfile(path):
                failed.append(f"{path} : ファイルが存在しません")
                continue
            try:
                ...
                failed.append(f"{path} : {e}")
        if failed:
            self.progress_time_label.setText(f"移動エラー: {len(failed)}件\n" + "\n".join(failed))
        else:
            self.progress_time_label.setText("選択したファイルを指定フォルダへ移動しました")
        self.find_duplicates()
