...
import hashlib
import subprocess
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QFileDialog, QListWidget, QMessageBox, QScrollArea, QGroupBox, QProgressBar, QInputDialog, QDialog, QGridLayout, QLineEdit, QDialogButtonBox, QListWidgetItem, QProgressDialog
...
class DuplicateFinderGUI(QWidget):
    ...
    def init_ui(self):
        ...
        self.video_broken_btn = QPushButton("動画壊れ検出")
        self.video_broken_btn.setStyleSheet("font-size:16px;color:#ff4444;border:2px solid #ff4444;border-radius:8px;padding:8px;")
        self.video_broken_btn.clicked.connect(self.show_video_broken_dialog)
        # --- デジタル修復ボタンを上部に追加 ---
        self.digital_repair_btn = QPushButton("デジタル修復")
        self.digital_repair_btn.setStyleSheet("font-size:16px;color:#ff44ff;border:2px solid #ff44ff;border-radius:8px;padding:8px;margin-top:0px;")
        self.digital_repair_btn.clicked.connect(lambda: self.show_digital_repair_dialog())
        btn_hbox = QHBoxLayout()
        btn_hbox.addWidget(self.dup_check_btn)
        btn_hbox.addWidget(self.face_group_btn)
        btn_hbox.addWidget(self.mp4_tool_btn)
        btn_hbox.addWidget(self.video_broken_btn)
        btn_hbox.addWidget(self.digital_repair_btn)
    ...
    def face_grouping_and_move(self):
        ...
        for idx, f in enumerate(all_files):
            if f.lower().endswith(video_exts):
                thumb = get_video_thumbnail(f, size=(80,80), error_files=error_files)
                enc = get_video_face_encoding(f)
            else:
                thumb = get_image_thumbnail(f, size=(80,80))
    ...
    def run_mp4_digital_repair(self, file_path=None):
        # デジタル修復処理（エラー隠蔽＋フレーム補間）
        if not file_path:
            file_path, _ = QFileDialog.getOpenFileName(self, "デジタル修復したい動画ファイルを選択", "", "動画ファイル (*.mp4 *.avi *.mov *.mkv *.wmv *.flv *.webm *.mpg *.mpeg *.3gp)")
            if not file_path:
                return
        ...
        try:
            # エラー隠蔽＋フレーム補間（再エンコード）
            cmd = [
                "ffmpeg", "-y", "-err_detect", "ignore_err", "-i", file_path,
                "-vf", "minterpolate=fps=30:mi_mode=mci:mc_mode=aobmc:vsbmc=1",
                "-c:v", "libx264", "-c:a", "aac", save_path
            ]
            dlg = QProgressDialog("デジタル修復中...", None, 0, 0, self)
            dlg.setWindowTitle("デジタル修復")
            dlg.setWindowModality(Qt.WindowModal)
            dlg.show()
            QApplication.processEvents()
            result = subprocess.run(cmd, capture_output=True, text=True, encoding="utf-8", errors="replace")
            ...
            if result.returncode == 0:
                ...
                QMessageBox.warning(self, "エラー", f"デジタル修復に失敗しました:\n{result.stderr}")
        except Exception as e:
            QMessageBox.warning(self, "エラー", f"デジタル修復処理中にエラー:\n{e}")

    def show_video_broken_dialog(self):
        # フォルダ選択
    ...
    def show_broken_video_groups(self, groups):
        ...
        for status, files in groups.items():
            ...
            for idx, f in enumerate(files):
                vbox = QVBoxLayout()
                thumb_img = get_video_thumbnail(f, size=(140,140))
                if thumb_img is not None:
                    rgb_img = thumb_img.convert("RGB")
                    w, h = rgb_img.size
                    data = rgb_img.tobytes()
