...
# --- サムネイルキャッシュ ---
THUMB_CACHE_FILE = ".thumb_cache.pkl"
thumb_cache = {}
...
def load_thumb_cache():
    global thumb_cache
    try:
        with open(THUMB_CACHE_FILE, "rb") as f:
            thumb_cache = pickle.load(f)
    except Exception:
        thumb_cache = {}

def save_thumb_cache():
    with thumb_cache_lock:
        try:
            with open(THUMB_CACHE_FILE, "wb") as f:
                pickle.dump(thumb_cache, f)
        except Exception:
            pass

def get_video_thumbnail(path, size=(140,140), error_files=None):
    key = (path, size)
    with thumb_cache_lock:
    ...
    try:
        ...
        pil_img = Image.fromarray(img).resize(size, Image.LANCZOS)
        with thumb_cache_lock:
            thumb_cache[key] = pil_img
        return pil_img
    except Exception:
        if error_files is not None:
            error_files.append(path)
...
def get_image_thumbnail(path, size=(140,140)):
    key = (path, size)
    with thumb_cache_lock:
        if key in thumb_cache:
            return thumb_cache[key]
...
def get_video_thumbnail(filepath, size=(240,240), error_files=None):
    ...
    try:
        ...
        with thumbnail_lock:
            thumbnail_cache[key] = pil_img.copy()
        return pil_img
    except Exception:
        if error_files is not None:
            error_files.append(filepath)
...
class DuplicateFinderGUI(QWidget):
    ...
    def find_duplicates(self):
        ...
        for idx, f in enumerate(image_files):
            ...
            thumb_q.put((f, (140,140), False, error_files))
