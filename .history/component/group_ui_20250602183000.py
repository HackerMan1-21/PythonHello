...
from PyQt5.QtCore import QSize, Qt, ContextMenuPolicy
...
def create_duplicate_group_ui(group, get_thumbnail_for_file, detail_cb, delete_cb, compare_cb, thumb_cache=None, defer_queue=None, thumb_widget_map=None, parent=None):
    ...
    for idx, f in enumerate(group):
        ...
        thumb_btn.clicked.connect(lambda _, path=f: detail_cb(parent, path) if parent is not None else detail_cb(path))
        ...
        path_label.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu if hasattr(Qt, 'ContextMenuPolicy') else Qt.CustomContextMenu)
        def show_context_menu(pos):
            from PyQt5.QtWidgets import QMenu
            menu = QMenu()
            open_action = menu.addAction("フォルダを開く")
            action = menu.exec_(path_label.mapToGlobal(pos))
            if action == open_action:
                import os, subprocess
                folder = os.path.dirname(f)
                if os.path.exists(folder):
                    subprocess.Popen(f'explorer "{folder}"')
        path_label.customContextMenuRequested.connect(show_context_menu)
    ...
    for idx, f in enumerate(error_files):
        ...
        def show_context_menu(pos):
            ...
            if action == open_action:
                ...
                if os.path.exists(folder):
                    subprocess.Popen(f'explorer "{folder}"')
        path_label.setContextMenuPolicy(3)  # Qt.CustomContextMenu = 3
        path_label.customContextMenuRequested.connect(show_context_menu)
        del_btn = QPushButton("削除")
        del_btn.setStyleSheet("font-size:12px;color:#ff00c8;max-width:180px;")
        del_btn.setFixedWidth(180)
