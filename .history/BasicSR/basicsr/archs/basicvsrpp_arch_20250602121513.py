    ...
    def __init__(self,
        ...
        if is_low_res_input:
            ...
            self.feat_extract = nn.Sequential(
                nn.Conv2d(3, mid_channels, 3, 2, 1), nn.LeakyReLU(
                    negative_slope=0.1, inplace=True),
                nn.Conv2d(mid_channels, mid_channels, 3, 2, 1), nn.LeakyReLU(
                    negative_slope=0.1, inplace=True),
        ...
        # upsampling module
        self.reconstruction = ConvResidualBlocks(
            5 * mid_channels, mid_channels, 5)

        self.upconv1 = nn.Conv2d(
            mid_channels, mid_channels * 4, 3, 1, 1, bias=True)
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i > 0 and self.is_with_alignment:
                ...
                if i > 1:  # second-order features
                    ...
                    flow_n2 = flow_n1 + \
                        flow_warp(flow_n2, flow_n1.permute(0, 2, 3, 1))
            ...
            feat = [feat_current] + [feats[k][idx]
                                     for k in feats if k not in ['spatial', module_name]] + [feat_prop]
            if self.cpu_cache:
...
class SecondOrderDeformableAlignment(ModulatedDeformConvPack):
    ...
    def forward(self, x, extra_feat, flow_1, flow_2):
        ...
        offset_1 = offset_1 + \
            flow_1.flip(1).repeat(1, offset_1.size(1) // 2, 1, 1)
        offset_2 = offset_2 + \
            flow_2.flip(1).repeat(1, offset_2.size(1) // 2, 1, 1)
        ...
        if LooseVersion(torchvision.__version__) >= LooseVersion('0.9.0'):
            return torchvision.ops.deform_conv2d(
                ...
                (self.stride, self.stride), (self.padding, self.padding), (self.dilation, self.dilation), mask
            )
        else:
            raise NotImplementedError('modulated_deform_conv is not available in this environment.')
