...
class BasicVSR(nn.Module):
    ...
    def __init__(self, num_feat=64, num_block=15, spynet_path=None):
        ...
        # propagation
        self.backward_trunk = ConvResidualBlocks(
            num_feat + 3, num_feat, num_block)
        self.forward_trunk = ConvResidualBlocks(
            num_feat + 3, num_feat, num_block)
    ...
    def forward(self, x):
        ...
        for i in range(0, n):
            ...
            base = F.interpolate(x_i, scale_factor=4,
...
class ConvResidualBlocks(nn.Module):
    ...
    def __init__(self, num_in_ch=3, num_out_ch=64, num_block=15):
        ...
        self.main = nn.Sequential(
            nn.Conv2d(num_in_ch, num_out_ch, 3, 1, 1, bias=True), nn.LeakyReLU(
...
class IconVSR(nn.Module):
    ...
    def __init__(self,
        ...
        self.edvr = EDVRFeatureExtractor(
            temporal_padding * 2 + 1, num_feat, edvr_path)
        # alignment
        self.spynet = SpyNet(spynet_path)

        # propagation
        self.backward_fusion = nn.Conv2d(
            2 * num_feat, num_feat, 3, 1, 1, bias=True)
        self.backward_trunk = ConvResidualBlocks(
            num_feat + 3, num_feat, num_block)
        ...
        self.forward_fusion = nn.Conv2d(
            2 * num_feat, num_feat, 3, 1, 1, bias=True)
        self.forward_trunk = ConvResidualBlocks(
            2 * num_feat + 3, num_feat, num_block)
...
class EDVRFeatureExtractor(nn.Module):
    ...
    def __init__(self, num_input_frame, num_feat, load_path):
        ...
        self.feature_extraction = make_layer(
            ResidualBlockNoBN, 5, num_feat=num_feat)
        ...
        self.fusion = TSAFusion(
            num_feat=num_feat, num_frame=num_input_frame, center_frame_idx=self.center_frame_idx)
    ...
    def forward(self, x):
        ...
        for i in range(n):
            nbr_feat_l = [  # neighboring feature list
                feat_l1[:, i, :, :, :].clone(), feat_l2[:, i, :, :,
                                                        :].clone(), feat_l3[:, i, :, :, :].clone()
            ]
